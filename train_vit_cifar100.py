...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        with open('/content/drive/MyDrive/vit_project/logs/vit_cifar100_metrics.txt', 'a') as f:
            f.write(f"{epoch},{train_loss:.4f},{train_acc:.2f},{test_loss:.4f},{test_acc:.2f}\n")
        
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), 'experiments/vit_cifar100_best.pth')
            
        torch.save(
        model.state_dict(),
        f'/content/drive/MyDrive/vit_project/experiments/vit_cifar100_epoch_{epoch+1}.pth'
        )
